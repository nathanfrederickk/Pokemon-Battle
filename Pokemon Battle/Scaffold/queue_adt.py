...
class CircularQueue(Queue[T]):
    ...
    def __str__(self) -> str:
        """ Outputs a string that consist of every item
        in the CircularQueue from the first in to the last in.
        ...
        for _ in range(len(self)):
            if index != (self.rear - 1) % len(self.array):
                res += str(self.array[index]) + ', '
            else:
                res += str(self.array[index])
