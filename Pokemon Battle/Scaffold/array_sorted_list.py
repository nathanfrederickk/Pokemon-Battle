...
class ArraySortedList(SortedList[T]):
    ...
    # Additional methods
    def optimized_add(self, item: ListItem) -> None:
        """ Add new element to the list and place it in order according to
        index returned by method _optimized_index_to_add.

        n = len(self)
        Time complexity for best case is O(1), and worst case is O(n).
        """
        if self.is_full():
            self._resize()

        # find where to place it
        position = self._optimized_index_to_add(item)

        self[position] = item
        self.length += 1

    def _optimized_index_to_add(self, item: ListItem) -> int:
        """ Find the position where the new item should be placed according
        to each key and if the keys are of the same value, the new item will
        be placed according to priority:
        ...
        for i in range(len(self)):
            if item.key == self[i].key:
                if item.value.PRIORITY > self[i].value.PRIORITY or item.value.PRIORITY == self[i].value.PRIORITY:
                    pos = i + 1
                else:
                    return i
            elif item.key > self[i].key:
                pos = i + 1
            elif item.key < self[i].key:
                return i
        return pos
    ...
    def __str__(self) -> str:
        ...
        Time complexity for best and worst case is O(m*n) where m is size of
        string and n is the size of list.
        """
        result = ''
        for i in range(len(self)):
            result = str(self[i].value) + ", " + result
